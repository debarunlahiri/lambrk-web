...
import { VideoCardSkeleton, BitCardSkeleton, PostCardSkeleton } from "./components/Shimmer";
import { VIDEOS, BITS, POSTS, shuffleArray } from "./constants/content";
...
export default function Home() {
  ...
  const [bits, setBits] = useState(BITS.slice(0, 4));
  const [posts, setPosts] = useState(POSTS.slice(0, 4));

  useEffect(() => {
    // Shuffle arrays only on client-side to prevent hydration mismatch
    setVideos(shuffleArray(VIDEOS));
    setBits(shuffleArray(BITS).slice(0, 4));
    setPosts(shuffleArray(POSTS).slice(0, 4));
    setIsLoaded(true);
  }, []);
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-semibold text-white">Bitz</h2>
                  <Link
                    href="/bits"
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mt-1">
                                <span className="text-xs text-gray-500">
                                  {bit.likes > 1000
                                    ? `${(bit.likes / 1000).toFixed(1)}K`
                                    : bit.likes}{" "}
                                  likes
                                </span>
                                <span className="text-xs text-gray-500">
                                  {bit.comments > 1000
                                    ? `${(bit.comments / 1000).toFixed(1)}K`
                                    : bit.comments}{" "}
                                  comments
                                </span>
                              </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 pt-2 border-t border-gray-700">
                        ...
                        <button className="flex items-center gap-1 text-gray-400 hover:text-red-400 transition-colors">
                          ...
                          <span className="text-xs">
                            {post.dislikes > 1000
                              ? `${(post.dislikes / 1000).toFixed(1)}K`
                              : post.dislikes}
                          </span>
                        </button>
                        <button className="flex items-center gap-1 text-gray-400 hover:text-blue-400 transition-colors">
